...
export function FunnelSkeleton() {
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-5 md:p-6 border border-gray-100/80 space-y-4">
      <div className="h-4 w-40 bg-gray-200 rounded-full animate-pulse" />
      {[100, 84, 62, 36, 24, 9].map((w, i) => (
        <div key={i} className="flex items-center gap-3">
          <div className="w-32 h-4 bg-gray-200 rounded animate-pulse" />
          <div className="flex-1 h-8 bg-gray-200 rounded-lg animate-pulse" style={{ width: `${w}%` }} />
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <div className="flex gap-2">
        {[1, 2, 3].map((i) => (
          <div key={i} className="h-9 w-16 bg-gray-200 rounded-xl animate-pulse" />
        ))}
      </div>
...
// ── Analytics: top content skeleton ──────────────────────────────────────────

export function TopContentSkeleton() {
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      {[1, 2, 3, 4].map((c) => (
        <div key={c} className="bg-white rounded-xl sm:rounded-2xl shadow-lg p-4 sm:p-5 md:p-6 border border-gray-100/80 space-y-3">
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="bg-white rounded-xl shadow-lg p-4 sm:p-5 border border-gray-100/80 space-y-3 animate-pulse">
